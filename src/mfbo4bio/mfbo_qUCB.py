...
def run(
    ...
    using the qUCB acquisition function.
    ...
    output_name : str, optional
        The base name for the output JSON file. Defaults to "qUCB_results".
    seed : int, optional
    ...
    beta : float, optional
        The beta parameter for the qUCB acquisition function,
        controlling the exploration-exploitation tradeoff. Defaults to 1.
    ...
    try:
        ...
        for iteration in range(n_iterations):
            ...
            for i in fidelity_level:
                ...
                # Covariances between original
                # and high-fidelity points (diagonal of upper-right block)
                covs = cov_matrix[:N, N:].diagonal()

                # Compute correlations
                correlations = covs / torch.sqrt(var_low * var_high)  # shape: (N,)
        ...
        with open(
            f"./saved_runs/bo/{clone_distribution}/qUCB/{output_name}.json", "w"
        ) as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--json_output", type=str, default="qUCB_results", help="Output JSON file"
    )
